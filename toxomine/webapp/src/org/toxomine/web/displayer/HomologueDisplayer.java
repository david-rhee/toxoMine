...
public class HomologueDisplayer extends ReportDisplayer
{
    ...
    public void display(HttpServletRequest request, ReportObject reportObject) {
        Map<String, Set<ResultElement>> homologues = new TreeMap<String, Set<ResultElement>>();
        ...
        while (it.hasNext()) {
            ...
            try {
                if (homologueObject.getFieldValue("symbol") != null) {
                    ResultElement re = new ResultElement(homologueObject, symbolPath, true);
                    Util.addToSetMap(homologues, organism.getShortName(), re);
                } else {
                    ResultElement re = new ResultElement(homologueObject, primaryIdentifierPath, true);
                    Util.addToSetMap(homologues, organism.getShortName(), re);
                }
    ...
    private static PathQuery getQuery(InterMineAPI im, Integer geneId, Set<String> dataSets) {
        ...
        q.addViews("Gene.homologues.homologue.organism.shortName", "Gene.homologues.homologue.primaryIdentifier");
