...
public class FeaturesAction extends InterMineAction
{
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("submission".equals(type)) {
            ...
            q.addView("SequenceFeature.primaryIdentifier");
            q.addView("SequenceFeature.score");
            q.addConstraint(Constraints.eq("SequenceFeature.submissions.TCid", tcId));
        }
        ...
        if ("results".equals(action)) {
            ...
            q.addView("SequenceFeature.id");
            tcId = request.getParameter("submission");
            q.addConstraint(Constraints.eq("SequenceFeature.submissions.TCid", tcId));

            Profile profile = SessionMethods.getProfile(session);

            String bagName = ("submission_" + tcId) + "_SequenceFeature_features";
            bagName = NameUtil.generateNewName(profile.getSavedBags().keySet(), bagName);
            BagHelper.createBagFromPathQuery(q, bagName, q.getDescription(), "SequenceFeature", profile, im);
